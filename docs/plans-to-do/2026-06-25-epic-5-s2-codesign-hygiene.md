...
- **Notarization itself** — the `notarytool submit` run is the deliberate, Adam-in-the-loop step (needs the Apple secrets in `release-pipeline.md:44`). Do not trigger a real notarization.
- **Hardened runtime / entitlements changes** (that's E5.S3 — `prd.md:217`). Leave `entitlements.plist` as-is; only flag if a signing failure *names* a missing entitlement.
