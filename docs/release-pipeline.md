...
| --- | --- | --- | --- |
| `Marquee` (Tauri shell) | `Contents/MacOS/Marquee` | Tauri 2.x bundler, via `APPLE_SIGNING_IDENTITY` + `macOS.signingIdentity` in `tauri.conf.json`, with hardened runtime + the (empty) `app-entitlements.plist` | It's the app's main executable; Gatekeeper checks it first. |
